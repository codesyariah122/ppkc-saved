				...
				<b-jumbotron class="shadow-none bg-gray">
					...
					<b-container class="mt-5">
						<b-row v-if="loading_event" class="row justify-content-center">
							<b-col v-for="(item, index) in pelatihans" :key="item.id" cols="4">
								<b-card>
									<b-row>
										<b-col>
						...
						<b-row v-else>
							<b-col v-if="listIndex <= pelatihans.length" v-for="listIndex in listToShow" cols="4">
						...
						<b-row v-if="listToShow <= pelatihans.length" class="mt-5 row justify-content-center">
							<b-col cols="12">
								<b-button block pill variant="primary" @click="ShowMore">Button</b-button>
							</b-col>
						</b-row>
					</b-container>
