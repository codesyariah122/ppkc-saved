              ...
              <div v-for="(c, index) in item.categories"  :key="c.id">
                ...
                <b-collapse :id="`collapse-${c.id}-inner`" class="collapse__category-event-2 mb-5">
                  <div v-for="(d, index) in c.details" :key="d.id" class="list__modul">
                    <mdb-btn v-b-toggle="`collapse-${d.id}-pelatihan`" size="sm" class="btn__category-2" @click="ToggleFile(d.kategori)">
                      <mdb-row class="row justify-content-between">
                        <mdb-col md="6">
          ...
          <div v-else>
            <div v-if="show_file">
              ...
              <div v-if="type == 1">
                ...
                aspect="16by9"
                :src="detailed.video"
                allowfullscreen class="embed__file"
                ></b-embed>
              </div>

              <div v-else-if="type == 2">
                <object :data="detailed.file_pdf" type="application/pdf" width="90%" :height="`${$device.isDesktop ? '800px' : '500px'}`">
                </object>
              </div>

              <div v-else-if="type == 3">
                <EventTestPreTest :id_test="id_test"/>
              </div>
