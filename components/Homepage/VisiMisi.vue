	...
	<div class="visi__misi" :style="`background-image: url(${bg_image});`">
		<mdb-container>
			...
			<mdb-row class="row visi-misi__content">
				<mdb-col class="mb-5" sm="3" md="3" lg="3">
					<h1>Visi</h1>
					<p v-if="context.visi">{{context.visi}}</p>
					<br>
					<h1>Misi</h1>
					<ol v-if="context.misi">
						<li v-for="n in context.misi.length" v-if="n % 2 == 1">{{context.misi[n]}}</li>
					</ol>
				</mdb-col>
		...
		methods: {
			...
			ContextData(){
				...
				.then((res) => {
					...
					const split = res.visi_misi.misi.split(".", res.visi_misi.misi.split(".").length-1)
