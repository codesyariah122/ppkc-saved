<template>
  ...
  <div v-else>
      ...
      <mdb-col lg="6" xs="6" sm="12" class="event__info">
        ...
        <mdb-row col="12">
          <mdb-col md="6" sm="4">
            <h5>Waktu</h5>
            <p>
              {{ $moment(details.tanggal_awal).format("LL") }} -
              {{ $moment(details.tanggal_akhir).format("LL") }}
            </p>
          </mdb-col>
          <mdb-col md="6" sm="4">
            <h5>Nilai SKP</h5>
        ...
        <mdb-row class="mt-3" col="12">
          <mdb-col
            v-if="status_pendaftaran == 'Daftar'"
            md="12"
            xs="12"
            sm="12"
          >
            ...
            <div v-else>
              <mdb-row class="row justify-content-start">
                <mdb-col md="12" col="12" sm="12">
                  <mdb-btn
                    @click="BeliKelas(details.kegiatan_id, details)"
                    class="btn my__btn-primary rounded-pill btn-block mb-2"
                    :size="`${$device.isDesktop ? 'md' : 'sm'}`"
                  >
                    <mdb-icon icon="credit-card" size="lg" />&nbsp;{{
          ...
          <mdb-col
            v-else-if="status_pendaftaran == 'Menunggu Konfirmasi'"
            md="12"
            xs="12"
            sm="12"
            class="mt-5"
          >
            ...
            <div v-else>
              <mdb-btn
                @click="MenungguKonfirmasi(details.kegiatan_id)"
          ...
          <mdb-col
            v-else-if="status_pendaftaran == 'Menunggu Pembayaran'"
            md="12"
            xs="12"
            sm="12"
            class="mt-5"
          >
            ...
            <div v-else>
              <mdb-btn
                @click="CheckPembayaran(details.kegiatan_id)"
                class="btn my__btn-secondary rounded-pill btn-block"
                :size="`${$device.isDesktop ? 'md' : 'sm'}`"
              >
                <mdb-icon
                  icon="calendar-check"
          ...
          <mdb-col v-else md="12" xs="12" sm="12" class="mt-5">
            ...
            <div v-else>
              <mdb-btn
                @click="
                  ProfileEvent(
                    $username(profiles.nama),
                    data_event.event_id,
                    $slug(details.kegiatan_title)
                  )
                "
                v-if="data_event || token.accessToken"
                class="btn my__btn-primary rounded-pill btn-block shadow-none"
                :size="`${$device.isDesktop ? 'md' : 'sm'}`"
              >
                Lanjut Pelatihan &nbsp;
                <mdb-icon icon="chalkboard-teacher" size="lg" />
              </mdb-btn>
            </div>
            <mdb-btn
              v-else
    ...
    <mdb-row style="margin-top: 64px">
      <mdb-col col="12">
        <h1>Pengumuman</h1>
      </mdb-col>
      <mdb-col col="12" class="mt-2" style="white-space: pre-wrap">
        <p style="font-size: 1em">{{ details.pengumuman }}</p>
      </mdb-col>
    </mdb-row>
          ...
          <div class="accordion shadow-none" role="tablist">
            ...
            >
              ...
              <b-collapse
                v-if="urutan === index + 1"
                v-model="visible"
                class="shadow-none schedule__collapse"
                :id="`accordion-1`"
              ...
              >
                <b-card-body>
                  <b-list-group class="list__categori">
                    <b-list-group-item
                      class="list__categori-item"
                      v-for="(categori, index) in item.categories"
                      :key="categori.urutan"
                    >
                      <h6>{{ categori.urutan }}. {{ categori.title }}</h6>
                      <b-list-group class="list__detail mt-2">
                        <b-list-group-item
                          v-for="(detail, index) in categori.details"
                          :key="detail.urutan"
                          class="list__detail-item mb-4"
                        >
                          <mdb-row class="justify-content-start">
                            <mdb-col md="10" sm="10" col="10" class="col-md-11">
                              <mdb-icon
                                class="text-success"
                                :icon="FilterIcon(detail.kategori)"
                                :size="`${$device.isDesktop ? 'md' : 'sm'}`"
                              />
...
export default {
  ...
  methods: {
    ...
    MenungguKonfirmasi(id) {
      const data_storage = {
        data: localStorage.getItem("success")
          ? JSON.parse(localStorage.getItem("success"))
          : "",
        message:
          "Proses pembayaran Anda sedang di check oleh admin kami. Anda dapat mengakses kelas yang Anda beli, Setelah pembayaran Anda di verifiksi oleh sistem kami.",
      };
