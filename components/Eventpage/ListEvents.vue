  ...
  <div class="mt-2 card__content-events">
    <mdb-container>
      <mdb-row
        v-if="loading"
        col="12"
        class="d-flex justify-content-center align-items-stretch mb-5 mt-5"
      >
      ...
      <mdb-row
        v-else
        col="12"
        class="d-flex justify-content-center align-items-stretch mb-5 webinar__content"
      >
        <mdb-col v-if="empty || error_search" lg="12" xs="12" sm="12">
          <mdb-alert
            :color="`${error_search ? 'danger' : 'info'}`"
            class="text-center"
          >
          ...
          <mdb-card>
            ...
            <mdb-card-body>
              <mdb-badge
                ...
                style="color: #004899 !important"
                >{{ list.kategori_value }}</mdb-badge
              >

              <mdb-card-title class="truncate">{{
                list.kegiatan_title
              }}</mdb-card-title>
              <span
                >{{ $moment(list.tanggal_awal).format("LL") }} -
                {{ $moment(list.tanggal_akhir).format("LL") }}</span
              >
              ...
              <div v-if="$route.path === '/events'">
                <nuxt-link
                  ...
                  class="btn btn-outline-primary mt-5 mb-2 btn-block"
                  color="primary"
              ...
              <div v-else>
                <mdb-btn
                  ...
                  size="md"
                  class="btn btn-outline-primary mt-5 mb-2 btn-block"
                  @click="RegistrasiEvent(list.kegiatan_id)"
                  >Daftar</mdb-btn
                >
...
export default {
  ...
  mounted() {
    this.VenoBox(), this.startTimer(), console.log(this.error_search);
  },
