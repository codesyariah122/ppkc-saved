<template>
	<div>
		...
		<div id="table-rtl" class="table-responsive">
			<table class="table table-hover">
				...
				<tbody>
					...
					<tr v-else v-for="(item, index) in data.rows" :key="item.id">
						...
						<td> {{$moment(item.jam_awal, "HH:mm:ss").format("HH:mm:ss")}} </td>
						<td> {{$moment(item.jam_akhir, "HH:mm:ss").format("HH:mm:ss")}} </td>
						...
						<td v-else class="add__data">
							<b-button v-b-modal="`modal-${item.id}`" size="sm" :style="style" ref="btnShow">Add</b-button>
						</td>
					</tr>
				</tbody>
				...
				<!-- <tfoot>
					<tr>
						<th colspan="12">
							<b-button v-b-modal="'addmodal'" size="sm" :style="style" ref="btnShow">
								Add Data
							</b-button>
						</th>
					</tr>
			...
			<b-modal v-for="(item, index) in data.rows" :key="item.id"  :id="`modal-${item.id}`" hide-footer>
				<b-row>
					<b-col  cols="12">
						<h4 class="card-title">
							{{item.title}}
						...
						<form @submit.stop.prevent="SendingData(item.id)">
							<div class="form-group">
								...
								<textarea id="sasaran" v-model="data.form.sasaran" class="form-control">
								</textarea>
							</div>
							<div class="form-group">
								<label for="sasaran">
								...
								<textarea id="keterangan" v-model="data.form.keterangan" class="form-control">
								</textarea>
							</div>
							<div v-if="item.sasaran == null && item.keterangan == null" class="form-group m-3">
...
<script>
	export default{
		props:['api_url', 'token', 'kegiatan_id', 'pelatihan_id', 'tests', 'evaluasi'],
		data(){
			return {
				data: {
					fields: [
				...
				loading_input: null,
				is_already: null,
				style: `${this.$device.isDesktop ? 'margin-top: -.3rem;' : 'margint-top:0;'}`
			}
		},

		mounted(){
			this.ListsData(),
			this.IsAlready(this.tests.is_already)
		},
